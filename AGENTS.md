...
## 十三、2026-06-13 微信/支付宝五类交易与退款冲减记录

- 微信、支付宝导入不再直接把 `收/支` 映射成账单方向。新增内部业务类型 `ImportTransactionKind = expense | income | transfer | refund | ignore`，统一由 `services/importTransactionClassifierService.ts` 在分类知识库之前判定。
- 状态过滤已统一：交易关闭、已关闭、支付失败、已取消、解冻成功、芝麻免押下单成功和预授权类交易归为 `ignore`；退款成功、已全额退款、已退款及退款描述归为 `refund`；成功状态继续进入微信/支付宝来源规则。
- 微信规则已覆盖零钱通转入/转出、零钱充值/提现、信用卡还款、理财通购买/赎回等 `transfer`；商户消费、二维码付款、外卖、交通、购物、话费、充电、12306 等归为 `expense`；群收款、二维码收款、他人转账和红包归为 `income`。
- 支付宝规则已覆盖账户存取、投资理财、余额宝自动转入/转出、实时提现、余额充值、花呗主动还款、信用借还和信用卡还款等 `transfer`；真实消费分类归为 `expense`；外部收款归为 `income`。花呗/信用卡消费仍记支出，还款只记内部调换，避免重复支出。
- `transfer` 和 `ignore` 不生成 `records` 账单，也不进入用户确认页；原生端会在 `import_rows` 保存内部类型和原始数据，保留导入审计。确定重复同样静默跳过，不再要求用户确认。
- 重复识别已提高自动跳过能力：同来源交易号或 `dedupe_hash` 一致直接跳过；同日、同金额、同内部类型且商户/备注相似度 `>= 0.82`，或相邻一天相似度 `>= 0.92`，升级为确定重复并静默跳过；中等相似度仍保留人工确认。
- 退款不再作为普通收入。退款账单使用收入方向兼容现有 `records.type` 约束，但额外保存 `transaction_kind = refund`；按交易号、商家订单号、金额、交易对方和商品说明匹配原支出，匹配成功时通过 `related_record_id` 关联。收入统计排除退款，支出、余额、趋势和预算已用金额按退款净额计算；未匹配退款仍作为独立“退款”记录展示，但不计入普通收入。
- 数据库已与业务逻辑对齐：`records` 新增 `merchant_order_no`、`transaction_kind`、`related_record_id`；`import_rows` 新增 `merchant_order_no`、`transaction_kind`。`records.type` / `import_rows.type` 继续保留 `income | expense` 兼容字段，避免破坏旧库和现有页面。
- 交易号唯一约束已改为 `import_provider + external_trade_no + transaction_kind`，允许同一订单的原消费与退款同时留痕，同时继续拦截同一种业务记录重复导入。
- 新增保守迁移 `drizzle/0003_skinny_menace.sql` 和对应快照。迁移只逐列增加字段、回填旧数据和更新索引，不包含 `DROP TABLE`、临时重建表或关闭外键。已用旧版最小 SQLite 数据库实际执行，旧 `expense` 数据成功回填 `transaction_kind = expense`，三列交易唯一索引验证通过。
- 新增 `npm run verify:import-rules` 回归脚本。规则用例 10/10 通过；真实微信样例解析 1091 行，分布为支出 672、收入 217、退款 103、内部调换 99、忽略 0；真实支付宝样例解析 268 行，分布为支出 172、收入 8、退款 26、内部调换 23、忽略 39，所有行均进入五类之一。
- 已运行 `F:\software\node.js\npm.cmd run typecheck`、`F:\software\node.js\npm.cmd run verify:import-rules`，均通过；`http://localhost:8081/import` 返回 200。Drizzle 再生成检查返回 `No schema changes`。

## 十四、2026-06-13 微信 5 月截图对账修复记录

- 用户提供的微信账单截图显示 2026 年 5 月微信原始口径为：支出 `6032.89` 元、收入 `3696.40` 元。已使用同账期真实 XLSX 逐行核对，共 221 行，XLSX 的原始 `收/支` 合计与截图完全一致。
- 定位并修复三个规则 bug：`转入零钱通-来自具体银行` / `零钱通转出-到具体银行` 未被通配识别为内部调换；原消费状态 `已退款(¥xx)` 被误判成退款行导致原支出消失；`群收款-转给他人`、扫二维码付款等支出因文本包含“收款”被错误覆盖为收入。
- 微信规则优先级调整为：先判断真正退款与内部调换，再以明确 `收/支=支出` 锁定支出；收入关键词只在收入方向生效。`已退款(¥xx)` 和支出方向的 `已全额退款` 保留为原消费，单独的正数退款流水才归为 `refund`。
- 零钱通规则改为模式匹配：所有 `转入零钱通-来自...`、`零钱通转出-到...` 均归为 `transfer`，不再只匹配固定“银行卡”文本。因此招商银行、农业银行等具体卡号流水不会误记收入。
- 修复后 2026 年 5 月五类金额为：`expense=6032.89`、`income=3381.52`、`refund=314.88`、`transfer=5302.10`、`ignore=0`。正常收入加退款为 `3696.40`，与微信截图收入一致；原始支出为 `6032.89`，与截图支出一致；按应用规则退款后净支出为 `5718.01`，正常收入为 `3381.52`，余额差额仍为 `-2336.49`，与微信原始收入减支出完全一致。
- 同一导入批次内的退款现在会在批量写库前匹配本批次原消费，并预分配记录 ID 写入 `related_record_id`；不再依赖原消费已经提前存在数据库。匹配支持交易号、商家订单号，以及金额、商户、备注和日期相似度，支持部分退款。
- 新增纯服务 `services/refundMatchingService.ts`，数据库历史消费匹配与同批次匹配使用同一算法。真实微信 5 月样例的 9 笔退款全部成功匹配原消费，结果为 `9/9`。
- 新增 `npm run verify:wechat-reconcile`，固定回归截图口径、五类金额、余额一致性和退款匹配率；`npm run verify:import-rules` 扩展到 13 个规则用例，覆盖退款原消费、具体银行卡转入零钱通和支出群收款。
- 已运行 `F:\software\node.js\npm.cmd run typecheck`、`F:\software\node.js\npm.cmd run verify:import-rules`、`F:\software\node.js\npm.cmd run verify:wechat-reconcile`，均通过。Playwright 使用真实微信 XLSX 完成浏览器文件选择与解析流程，页面无运行时或控制台错误。

## 十五、2026-06-13 支付宝 4 月/5 月截图对账修复记录

- 用户提供的支付宝截图口径为：2026 年 5 月支出 `408.40` 元、收入 `38.00` 元；2026 年 4 月支出 `787.52` 元、收入 `0.00` 元。修复前应用显示 5 月支出 `396.10`、4 月支出 `229.00`，差额分别为 `12.30` 和 `558.52`，恰好等于对应月份的退款总额。
- 根因是支付宝与微信的退款源数据不同：支付宝已将全额退款原订单标记为 `交易关闭`，原订单会归为 `ignore`，已经不进入支出；退款行作为独立 `refund` 保留。统计层此前对所有退款继续减支出，导致支付宝退款被二次冲减。
- 统计口径已修复：`refund` 继续不计入普通收入；只有存在 `related_record_id`、确实关联到已入账支出的退款才冲减支出。没有关联已入账支出的独立退款只保留展示和导入审计，不再修改支出合计。
- 支付宝退款匹配已收紧：支付宝存在稳定的商家订单号时必须精确匹配同一订单；订单号未在已入账支出中命中时禁止再用模糊商户/金额匹配，避免把退款错误关联到其他正常消费。微信退款继续允许高置信模糊匹配，因为微信退款与原消费的交易号/商户单号并不稳定一致。
- 新增 `npm run verify:alipay-reconcile`。真实 CSV 对账结果：2026 年 4 月共 29 条源流水，生成支出 20 笔、退款 4 笔、内部调换 1 笔、忽略 4 笔，最终账单 24 笔，支出 `787.52`、收入 `0.00`；2026 年 5 月共 36 条源流水，生成支出 24 笔、收入 1 笔、退款 3 笔、内部调换 4 笔、忽略 4 笔，最终账单 28 笔，支出 `408.40`、收入 `38.00`。
- 支付宝 4 月 4 笔退款和 5 月 3 笔退款对应的原订单均为 `交易关闭`，因此退款关联结果分别为 `0/4`、`0/3`，符合“不对已排除原订单二次冲减”的预期。微信 5 月退款回归仍为 `9/9` 匹配，未被本次支付宝修复破坏。
- 已运行 `npm run verify:alipay-reconcile`、`npm run verify:wechat-reconcile`、`npm run verify:import-rules` 和 `npm run typecheck`，全部通过。Playwright 使用真实支付宝 CSV 完成浏览器导入后，账单页实测 2026 年 5 月显示收入 `38.00`、支出 `408.40`、28 笔；2026 年 4 月显示收入 `0.00`、支出 `787.52`、24 笔，页面无运行时或控制台错误。
